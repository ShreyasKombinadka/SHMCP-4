module Register_file (  // Register file
    input clk,
...
// Reister A block
regA A (.clk(clk), .grst(grst), .lrst(lrst),
        .imm(imm), .rs1(rs1_en), .rs2(rs2_en), .ws1(ws1_en), .bus(bus), .regA_disp(regA_disp)
);

// Register B block
regB B (.clk(clk), .grst(grst), .lrst(lrst),
        .imm(imm), .rs3(rs3_en), .rs4(rs4_en), .ws2(ws2_en), .bus(bus), .regB_disp(regB_disp)
);

// Register OP block
regOP OP (.clk(clk), .grst(grst), .lrst(lrst),
        .imm(imm), .rs5(rs5_en), .ws3(ws3_en), .bus(bus), .regOP_disp(regOP_disp)
);
