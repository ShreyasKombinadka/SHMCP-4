module ALU_unit (   // Arithmatic & Logic unit
    input clk,
...
assign rega_disp = a ;      // Regsiter data to show on the display
assign regb_disp = b ;      // Regsiter data to show on the display
assign regop_disp = op ;    // Regsiter data to show on the display
...
//-----------------------------------------------------
regR R (.clk(clk), .grst(grst), .lrst(rst_dec), .op_sel(op_sel_dec), .ws1(ws1_en), .ws2(ws2_en), .Z(z), .C(c), .O(o), .N(n), .res(result), .bus(bus),
        .regR_disp(regR_disp), .regF_disp(regF_disp));    // Result and Flag registers
